...
public class SingletonPropertyTest {
    ...
    public void without_dependencies() {
        TestIntrospectionImpl introspection = TestIntrospectionImpl.create(Scopes.ROOT)
                .add(new TestPropertyProvider<>(Properties.HELLO_WORLD, Priority.FALLBACK, SingletonPropertyTest.class, _ -> "Hello World"))
                .build();
    ...
    public void should_always_return_same_instance() {
        TestIntrospectionImpl introspection = TestIntrospectionImpl.create(Scopes.ROOT)
                .add(new TestPropertyProvider<>(Properties.TEST_STUB, Priority.FALLBACK, SingletonPropertyTest.class, _ -> new Properties.TestStub()))
                .build();
    ...
    public void should_not_return_fallback() {
        TestIntrospectionImpl introspection = TestIntrospectionImpl.create(Scopes.ROOT)
                .add(new TestPropertyProvider<>(Properties.HELLO_WORLD, Priority.FALLBACK, SingletonPropertyTest.class, _ -> "Hello World (Fallback)"))
                .add(new TestPropertyProvider<>(Properties.HELLO_WORLD, Priority.of(10), SingletonPropertyTest.class, _ -> "Hello World"))
                .build();
