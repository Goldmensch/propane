...
/// An event can only be fired from a [Introspection] instance with exactly the [scope][#scope]
/// of the that event.
///
/// For example, if we have an [Event] of scope `Scopes.CONFIGURATION`,
/// we can only use [IntrospectionImpl#publish(Event)] if [IntrospectionImpl#scope()] returns
